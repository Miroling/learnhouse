...
def _ensure_org_access(user_id: int, org_id: int, db_session: Session) -> Organization:
...
async def api_atlas_session(
    ...
    _ensure_org_access(current_user.id, org_id, db_session)
    token, row = create_atlas_session_token(db_session, current_user.id, org_id)
...
async def api_atlas_revoke(
    ...
    revoke_atlas_session_token(db_session, body.token_uuid, current_user.id)
    return {"ok": True}
...
async def api_atlas_chat(
    ...
    _ensure_org_access(current_user.id, body.org_id, db_session)
