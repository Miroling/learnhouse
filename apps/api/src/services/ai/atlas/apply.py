...
def _scalar_first(db, statement):
...
def _resolve_course_id(deps: AtlasDeps, course_uuid: str) -> int:
    course = _scalar_first(
...
async def _apply_create_chapter(edit: PendingEdit, deps: AtlasDeps) -> dict[str, Any]:
    ...
    if not course_id:
        ...
        course_id = _resolve_course_id(deps, parent_uuid)
...
async def _apply_create_activity(edit: PendingEdit, deps: AtlasDeps) -> dict[str, Any]:
    ...
    chapter = _scalar_first(deps.db, select(Chapter).where(Chapter.id == int(chapter_id)))
    if not chapter:
